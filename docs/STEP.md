...
The repo already contains a **Next.js dashboard** in `frontend/` and an **Express worker** in `backend/`. The UI follows the three-column “workflow builder + control chat” layout described in [DESIGN.md](./DESIGN.md): API/integrations rail (left), React Flow canvas (center), mission chat + specialist readout (right).
...
- create SKILL.md and README generation as first-class agent artifacts
...
- [x] Specialist agent config panel and dashboard (Config, SKILL.md, README, sample output)
- [x] Central Agent generates agent profile (worker stub in `backend/src/agent/mother-agent.ts`; internal filename kept for compatibility)
...
- one Central Agent flow
- one reusable agent package (profile + SKILL.md + README)
- one tool call
...
- vitals panel
- terminal drawer
- workspace rail (API/integration hints)
...
The specialist agent panel should show the agent generated by the Central Agent and expose editable fields for:
- agent name
- agent role
- agent purpose
- allowed tools
...
- SKILL.md playbook and README handoff
...
- empty state before the Central Agent runs
- generated state after the Central Agent returns a profile
...
- generate a specialist agent profile from the Central Agent
- turn it into steps
...
2. Central Agent breaks it into one or two steps
3. Central Agent generates a specialist agent package: profile, SKILL.md, README, tool refs
4. frontend shows the package in the agent dashboard
...
- SKILL.md visible as the main generated artifact
- sample deliverables clearly labeled as optional proof of work

The generated agent package should feel like a real artifact, not just a chat response. Users need to understand what the agent is, what it can do, what skills it has, and what they can still change by hand.
